...
#### Step 1. まずは定義 
確率変数$X \sim \mathcal{N}(\mu_x, σ_x^2)$および
...
中心極限定理より、平均値の特性として、$\bar{X} \sim \mathcal{N}(\mu_x, σ_x^2/m)$および
$\bar{Y} \sim \mathcal{N}(\mu_y, σ_y^2/n)$であるから、
期待値の性質から、$\bar{X}-\bar{Y} \sim \mathcal{N}(\mu_x - \mu_y, σ_x^2/m + σ_y^2/n)$である。

#### Step 3. 標本平均の差に関する点推定量
$\mathrm{E}[\bar{X} -\bar{Y}] = \mathrm{E}[\bar{X}] -\mathrm{E}[\bar{Y}] = \mu_x - \mu_y$である。 母平均$\mu_x$と$\mu_y$は未知なので、$\bar{X}$の点推定として、$x$の平均値$m_x$、$\bar{Y}$の点推定として、$y$の平均値$m_y$を推定値とする。
