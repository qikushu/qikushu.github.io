...
#### 標本から母分散を推定
標本の不偏分散 $ s^2 = \frac{(X_1 - \bar X)^2 + \cdots + (X_n - \bar X)^2}{n-1} $の期待値は、母分散$\sigma_X^2$と等しい。
...
すなわち、標本$X$の平均$\bar{X}$をとると、分散は$n$分の1になることを示し、多くのサンプルからの平均を取った方が母平均の推定値として安定する、ということを言っている。
