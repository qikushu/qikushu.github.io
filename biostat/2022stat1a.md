...
ans =
  50x5 table
    ...
    ___________    __________    ___________    __________    __________
        ...
        4.6           3.4            1.4           0.3        {'setosa'}
```
`fishertable.Species == 'setosa'`と書けそうだが、`fishertable.Species`はcell型、'setosa'は文字列(string)型であり、データ型が異なるので
直接比較演算子==にて比較できない。そこでcell型をstring型に変換するstring()関数を入れておく必要がある。ちょっと面倒。
### 演習問題
データフレームirisに対して、
